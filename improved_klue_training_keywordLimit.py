...
def main():
    ...
    train_losses, val_f1_scores, best_f1, best_model_path = train_model(
        model, train_loader, val_loader, timestamp, epochs=5, learning_rate=2e-5
    )
    ...
    # 9. 추론 테스트
    test_texts = [
        "아이들이 아파서 급하게 응급실에 갔을 때 진료 순서를 바꿔준 학생이 있었어요. 그때는 정신이 없었지만, 배려로 놀랐던 제 마음도 진정되고 도움을 받았어요.",
        "가능하면 사람을 미워하지 않으려고 해서 이건 없네요.", 
        "육아로 힘들 때 아이들이 해주는 엄마 사랑해요가 가장 컸었죠.",
        "관계에는 노력이 많이 필요하다는 점이요. 서로의 노력이 있어야 유지가 가능하고 오래가는 것 같아요."
    ]
