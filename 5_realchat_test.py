...
def create_dummy_data():
    """테스트를 위한 더미 답변 데이터 생성"""
    dummy_answers = {
        0: {
            "answer": "저는 젊었을 때 남산공원에 자주 갔어요. 봄에는 벚꽃이 아름다웠죠.",
            "date": date.today() - timedelta(days=35),
            "question": QUESTIONS[0]
        },
        1: {
            "answer": "파란색 카디건을 즐겨 입었어요. 아내가 선물해준 거였죠.",
            "date": date.today() - timedelta(days=34),
            "question": QUESTIONS[1]
        },
        2: {
            "answer": "명절에는 온 가족이 모여 윷놀이를 했어요. 막내가 항상 이겼죠.",
            "date": date.today() - timedelta(days=33),
            "question": QUESTIONS[2]
        },
        3: {
            "answer": "한강 둔치를 따라 산책하는 걸 좋아했어요. 석양이 참 아름다웠습니다.",
...
def memory_check_phase():
    ...
    if not st.session_state.stored_answers:
        st.warning("⚠️ 저장된 답변이 없습니다.")
        st.info("💡 테스트를 위해 사이드바에서 '🧪 테스트 데이터 생성' 버튼을 눌러주세요.")
        return
    ...
    # 새로운 질문을 완료했거나 이미 완료한 경우, 기억 점검 진행
        ...
        # 기억 점검을 이미 했다면 더 이상 진행하지 않음
        if st.session_state.daily_memory_checks >= 1:
            return
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🧪 테스트 데이터 생성"):
            st.session_state.stored_answers = create_dummy_data()
            st.session_state.current_question_index = 4  # 더미 데이터 이후부터 시작
