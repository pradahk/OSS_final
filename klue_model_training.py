...
# 새로운 버전의 transformers 라이브러리에서 AdamW 옵티마이저가 torch로 이동되었기 때문에 오류 발생
    ...
    example_data = [
        {
            ...
            "labels": ["B-KEY", "O", "B-KEY", "O", "B-KEY", "B-KEY", "O", "O"],
        ...
        },
        {
            ...
            "labels": ["B-KEY", "O", "B-KEY", "O", "B-KEY", "O", "O"],
...
def train_model(model, train_loader, val_loader, epochs: int = 5, learning_rate: float = 2e-5):
    """모델 학습"""
    ...
    for epoch in range(epochs):
        ...
        if val_f1 > best_f1:
            ...
            print(f"   🏆 최고 성능 모델 저장 (F1: {best_f1:.4f})")
...
def save_model(model, tokenizer, label2id, id2label, save_path: str = "klue_keyword_extractor"):
    """모델 및 설정 저장"""
    print(f"\n💾 모델 저장 중: {save_path}")
...
def plot_training_history(train_losses, val_f1_scores):
    """학습 히스토리 시각화"""
...
def main():
    """4단계 메인 실행 함수"""
    print("🎯 4단계: KLUE-BERT 키워드 추출 모델 학습")
    print("=" * 60)
    ...
    # 5. 모델 학습
    train_losses, val_f1_scores, best_f1 = train_model(
        model, train_loader, val_loader, epochs=5, learning_rate=2e-5
    )
    ...
    # 8. 모델 저장
    save_model(model, tokenizer, label2id, id2label)
    ...
    print(f"✅ 모델 저장: klue_keyword_extractor/")
    ...
    return model, tokenizer, final_f1
