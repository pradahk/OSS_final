...
class KeywordExtractor:
    ...
    def load_model(self, model_path: Optional[str] = None):
        ...
        try:
            ...
            if not os.path.exists(model_weight_path):
                 raise FileNotFoundError(f"모델 가중치 파일을 찾을 수 없습니다: {model_weight_path}")
            ...
            if config_path and os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    ...
                    self.id2label = {int(k): v for k, v in config.get("id2label", {0: "O", 1: "B-KEY", 2: "I-KEY"}).items()}
            else:
                # 기본값 사용
            ...
            st.error(f"❌ 모델 파일을 찾을 수 없어 키워드 추출기 초기화에 실패했습니다: {e}")
            self.model = None # 실패 시 모델을 None으로 설정
            raise  # 오류를 다시 발생시켜 상위 호출자에게 알림
        except Exception as e:
            st.error(f"❌ 모델 로드 중 예상치 못한 오류 발생: {e}")
            self.model = None
            raise
    ...
    def extract_keywords(self, text: str, max_keywords: Optional[int] = None) -> List[str]:
        ...
        if self.model is None or self.tokenizer is None:
            st.error("키워드 추출 모델이 로드되지 않았습니다. 추출을 진행할 수 없습니다.")
            return []
...
def get_keyword_extractor() -> KeywordExtractor:
    ...
    if _keyword_extractor is None:
        try:
            ...
            # UI 코드에서 get_keyword_extractor() 호출 후 None 체크 필요
            st.error("키워드 추출기 인스턴스를 생성하는데 실패했습니다. 앱의 일부 기능이 동작하지 않을 수 있습니다.")
